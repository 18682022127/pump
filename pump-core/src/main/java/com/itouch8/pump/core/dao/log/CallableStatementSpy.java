...
public class CallableStatementSpy extends PreparedStatementSpy implements CallableStatement {
    ...
    public <T> T unwrap(Class<T> iface) throws SQLException {
        ...
        try {
            ...
            // NOTE: could call super.isWrapperFor to simplify this logic, but it would result in
            // extra log output
            // because the super classes would be invoked, thus executing their logging methods
            // too...
            return (T) reportReturn(methodCall, (iface != null && (iface == CallableStatement.class || iface == PreparedStatement.class || iface == Statement.class || iface == Spy.class)) ? (T) this : realCallableStatement.unwrap(iface));
        } catch (SQLException s) {
            reportException(methodCall, s);
            throw s;
        }
